document.addEventListener('DOMContentLoaded',()=>{
  ...
  buttons.forEach(b=>b.addEventListener('click',()=>{buttons.forEach(x=>x.classList.remove('active'));b.classList.add('active');if(room){room.classList.remove('reading','energized','momentum');const m=b.dataset.mode;room.classList.add(m==='focus'?'reading':m==='energy'?'energized':'momentum');const cap=room.querySelector('.room-caption');if(cap)cap.textContent=copy[m]||copy.focus;const core=room.querySelector('.core');if(core)core.textContent=m.toUpperCase()}}));
});
